...
function sfnt2woff(sfnt) {
  ...
  for (i = 0; i < numTables; ++i) {
    ...
    console.log(csum, tableDirectoryEntry.checksum);
    ...
    var compLength =
      sfntSlice.length < compSfntData.length
        ? sfntSlice.length
        : compSfntData.length;
    ...
    WOFFTableDir.writeUInt32BE(
      tableDirectoryEntry.tag,
      i * WOFF_TABLE_DIR_SIZE
    );
    ...
    WOFFTableDir.writeUInt32BE(
      ...
      i * WOFF_TABLE_DIR_SIZE + 12
    );
    WOFFTableDir.writeUInt32BE(
      tableDirectoryEntry.checksum,
      i * WOFF_TABLE_DIR_SIZE + 16
    );
  ...
  var WOFFHeader = constructWOFFHeader(
    flavor,
    woffTableOffset,
    numTables,
    totalSfntSize
  );
  var WOFF = constructWOFF(
    WOFFHeader,
    WOFFTableDir,
    WOFFTableData,
    woffTableOffset
...
function woff2sfnt(woff) {
  ...
  var SFNTHeader = constructSFNTHeader(
    sfntVersion,
    numTables,
    searchRange,
    entrySelector,
  ...
  for (i = 0; i < numTables; ++i) {
    ...
    SFNTTableDir.writeUInt32BE(
      tableDirectoryEntry.tag,
      i * SFNT_TABLE_DIR_SIZE
    );
    SFNTTableDir.writeUInt32BE(
      tableDirectoryEntry.origChecksum,
      i * SFNT_TABLE_DIR_SIZE + 4
    );
    SFNTTableDir.writeUInt32BE(sfntTableOffset, i * SFNT_TABLE_DIR_SIZE + 8);
    SFNTTableDir.writeUInt32BE(
      tableDirectoryEntry.origLen,
      i * SFNT_TABLE_DIR_SIZE + 12
    );
    ...
    if (tableDirectoryEntry.origLen % 4 !== 0) {
      sfntDataEntry = fourByteAlignedBuffer(
        sfntDataEntry,
        tableDirectoryEntry.origLen
      );
